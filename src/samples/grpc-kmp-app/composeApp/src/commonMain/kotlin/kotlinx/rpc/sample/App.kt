...
fun App() {
    ...
    val grpcClient = remember { GrpcClient(SERVER_HOST, 8080) { credentials = plaintext() } }

    val service = remember { grpcClient.withService<MessageService>() }

    val httpClient = remember { HttpClient() }

    MaterialTheme { ChatScreen(service, httpClient) }
}
...
private fun ChatScreen(service: MessageService, httpClient: HttpClient) {
    ...
    Column(Modifier.fillMaxSize().windowInsetsPadding(WindowInsets.safeDrawing).padding(16.dp)) {
        ...
        LazyColumn(
            ...
            items(messages) { m -> MessageBubble(m, me) }
        }
...
@Composable
private fun HealthCheckButton(status: RestStatus, onCheck: () -> Unit) {
...
private fun Throwable.toRpcError(action: String): String {
    ...
    return status?.statusCode?.toErrorMessage(action) ?: message ?: "Unknown error while $action."
}
...
@Composable
fun ErrorBanner(error: String, retrySeconds: Long) {
    Column(
        modifier =
            Modifier.fillMaxWidth()
...
@Composable
@OptIn(ExperimentalTime::class)
fun MessageBubble(message: ChatEntry, me: String, modifier: Modifier = Modifier) {
    ...
    val local =
        Instant.fromEpochMilliseconds(message.tsMillis)
