...
That value is required by `deploy_android_app(...)` in `example/greeter/android/BUILD.bazel`.
...
./tools/deploy/verify_ci_permissions.sh
```

Or via Bazel:

```bash
bazel run //tools/deploy:verify_ci_permissions
```
...
Populate these values in `tools/deploy/dev_environment.json`:
- `gcp.project_id`
- `gcp.region`
...
Replace the placeholder Firebase App ID in `example/greeter/android/BUILD.bazel`.

After updating those values, validate the setup locally:

```bash
bazel run //example/greeter/kotlin:deploy -- --dry-run
bazel run //example/greeter/web:deploy -- --dry-run
bazel run //example/greeter/android:deploy -- --dry-run
```
