...
For this Bazel monorepo, add the Maven artifacts to `third_party/maven/libs.versions.toml`:
...
Repin the Bazel Maven repository:

    REPIN=1 bazel run @third_party_maven//:pin

Then add the generated labels to the relevant `BUILD.bazel` target:

    deps = [
        "@third_party_maven//:androidx_navigation3_navigation3_runtime",
        "@third_party_maven//:androidx_navigation3_navigation3_ui",

        # If using the ViewModel add-on library
        "@third_party_maven//:androidx_lifecycle_lifecycle_viewmodel_navigation3",
    ]
...
> [!NOTE]
> **Note:** The `@Serializable` annotation is provided by the KotlinX Serialization plugin. In this repo, wire it through the Bazel target's `plugins`, for example `//build/toolchains/kotlin/serialization:serialization_compiler_plugin`, and add `@third_party_maven//:org_jetbrains_kotlinx_kotlinx_serialization_json` where JSON serialization is needed.
