...
3. In this Bazel monorepo, verify `third_party/maven/libs.versions.toml` contains the serialization runtime:

    [libraries]
    kotlinx-serialization-json = { module = "org.jetbrains.kotlinx:kotlinx-serialization-json", version.ref = "kotlinx-serialization" }

Repin if you changed Maven coordinates:

    REPIN=1 bazel run @third_party_maven//:pin

- Add `//build/toolchains/kotlin/serialization:serialization_compiler_plugin` to the relevant `BUILD.bazel` target's `plugins`.
- Add `@third_party_maven//:org_jetbrains_kotlinx_kotlinx_serialization_json` to the target's `deps` when JSON serialization APIs are used.
